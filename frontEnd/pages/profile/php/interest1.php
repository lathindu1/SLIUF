...
$sql3="SELECT * FROM (SELECT * FROM tread_list WHERE Field_Id='".$interest1."' ORDER BY Tread_Id DESC LIMIT 3) t ORDER BY Tread_Id ASC";
...
$result3 = mysqli_query($conn,$sql3);
    while($row = mysqli_fetch_array($result3))
    {
        ...
        $comment="";
        $sql4 = "SELECT * FROM treads_comments WHERE tId=".$treadId;
        $result4 = mysqli_query($conn,$sql4);
        while ($row1 = mysqli_fetch_array($result4)) {
            $cUser = $row1['uId'];
        }
        $sql5="SELECT * FROM user_data WHERE index_number_fk= '".$indexnum."'";
